    ...
    def test_get_api_key_from_config_file(self, mock_home):
        ...
        with patch('builtins.open', mock_open(read_data=json.dumps(config_data))) as m:
            ...
            with patch('business_finder.config.Path.exists', return_value=True):
                ...
                m.assert_called_once_with(mock_home_dir / '.business_finder' / 'config.json', 'r')
        ...
        with patch('business_finder.config.Path.mkdir') as mock_mkdir:
            with patch('business_finder.config.Path.exists', return_value=False):
                with patch('builtins.open', mock_open()) as m:
                    ...
                    assert any('new_test_api_key' in arg for arg in write_args if isinstance(arg, str))
    ...
    def test_save_api_key_existing_file(self, mock_home):
        ...
        with patch('business_finder.config.Path.mkdir') as mock_mkdir:
            with patch('business_finder.config.Path.exists', return_value=True):
                ...
                with patch('builtins.open', mock_open(read_data=json.dumps(existing_config))) as m:
                    ...
                    write_args = [call[0][0] for call in handle.write.call_args_list]
                    assert any('updated_api_key' in arg for arg in write_args if isinstance(arg, str))
                    assert any('other_setting' in arg for arg in write_args if isinstance(arg, str))